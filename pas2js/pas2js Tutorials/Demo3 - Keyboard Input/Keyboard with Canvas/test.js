...
rtl.module("program",["System","browserconsole","Web"],function () {
  ...
  this.DrawBox = function () {
    ...
    $mod.ctx.fillRect($mod.x,$mod.y,30,30);
  };
  this.HandleKeyDown = function (k) {
    var Result = false;
    if (k.key === pas.Web.TJSKeyNames.ArrowLeft) $mod.x -= 5;
    if (k.key === pas.Web.TJSKeyNames.ArrowRight) $mod.x += 5;
    if (k.key === pas.Web.TJSKeyNames.ArrowDown) $mod.y += 5;
    if (k.key === pas.Web.TJSKeyNames.ArrowUp) $mod.y -= 5;
    $mod.DrawBox();
    return Result;
  };
  $mod.$main = function () {
    ...
    $mod.InitCanvas();
    $mod.DrawBox();
    pas.System.Writeln("Keyboard and Canvas");
