...
rtl.module("program",["System","browserconsole","Web"],function () {
  ...
  this.Draw = function () {
    ...
    $mod.ctx.fillRect(0,0,64,64);
    $mod.ctx.fillStyle = "white";
    $mod.ctx.fillText("Hello World",5,35);
    $mod.myImageData = new ImageData(64,64);
    $mod.myImageData = $mod.ctx.getImageData(0,0,64,64);
    $mod.ctx.putImageData($mod.myImageData,10,100);
    for (i = 0; i <= 16383; i++) {
      $mod.myImageData.data[i] = 128 & $mod.myImageData.data[i];
    };
    $mod.ctx.putImageData($mod.myImageData,100,100);
  };
  this.onLoadDraw = function (Event) {
    var Result = false;
    $mod.Draw();
    return Result;
  };
  $mod.$main = function () {
    window.onload = rtl.createSafeCallback($mod,"onLoadDraw");
    $mod.InitCanvas();
    pas.System.Writeln("get put imagedata");
  };
});
