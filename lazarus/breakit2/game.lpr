...
uses cmem, ray_header,collision,SysUtils, Timer;
...
begin
 GetShapeRec(shapeid,sr);
 DrawRectangle(sr.x,sr.y,sr.width,sr.height,BRICK_COLOR);
end;
...
begin
  DrawText('Triggered by Timer Code!', 10, 0, 20,RAYWHITE);
end;
...
procedure ChangeBrickColor(timerid : integer);
begin
  Case Brick_Color_Select of 1:Brick_Color:=YELLOW;
                             2:Brick_Color:=RED;
                             3:Brick_Color:=BLUE;
  end;
  inc(Brick_Color_Select);
  if Brick_Color_Select > 3 then Brick_Color_Select:=1;
end;
...
begin
 ...
 SetTimer(2,true,false,true,2,@ChangeBrickColor);
...
// ToggleFullscreen;
 ...
 begin
   ...
   BeginDrawing();
     ...
     DrawShapes;
     if Flash_Text then DisplayText;
   EndDrawing();
  end;
  CloseWindow();
 end;
