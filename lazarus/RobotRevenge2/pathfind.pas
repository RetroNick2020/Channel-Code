...
function GetGridDir(x,y : integer) : integer;
begin
 GetGridDir:=Grid[x,y].dir;
end;

procedure PlaceEnemy(x,y : integer);
begin
  if Grid[x,y].Dir <> G_Wall then GRID[x,y].Dir:=G_Enemy;
end;

procedure PlacePlayer(x,y : integer);
begin
 if Grid[x,y].Dir <> G_Wall then GRID[x,y].Dir:=G_Player;
end;
...
begin
 startdir:=0;
 cval:=30000;
 if (x > 0) AND  (Grid[x-1,y].dir<>G_Wall) then
 begin
    startdir:=G_LEFT;
    cval:=Grid[x-1,y].CVal;
 end;
 if (x < Max_Grid_x) and (Grid[x+1,y].dir<>G_Wall) then
 begin
 ...
 if (y > 0) and (Grid[x,y-1].dir<>G_Wall) then
 begin
 ...
 if (y < Max_Grid_Y) and (Grid[x,y+1].dir<>G_Wall) then
 begin
   if Grid[x,y+1].CVal < cval then
   begin
     startdir:=G_DOWN;
 ...
 FindDirectionFromEnemy:=startdir;
end;
